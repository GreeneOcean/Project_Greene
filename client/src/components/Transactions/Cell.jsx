...
const Cell = ({ status, group, user, item, setOther, clearOther }) => {
  ...
  return (
    <Container>
      ...
      <div onClick={() => clearOther()}>
        PRODUCT INFO: {item.title} {item.category} {item.description}{" "}
      </div>
      <div onClick={() => setOther(item.posted_by)}>
        POSTED BY INFO: {item.posted_by}{" "}
      </div>
